...
def find_backup_file(database, message, backup_dir='_backups'):
    ...
    logger.debug(f"No matching backup file found for database: {database} with message: {message}")
    return None
...
def add_restore_parser(subparsers):
    ...
    restore_parser.add_argument('--server', default=env_config["SERVER"], help="SQL Server instance")
    restore_parser.add_argument('--database', default=env_config["TARGET_DB"], help="Database to restore")
    restore_parser.add_argument('--message', help="Optional message to match in backup file")
    restore_parser.add_argument('--backup-dir', default='_backups', help="Directory where backup files are stored")
    restore_parser.add_argument('--virgin', action='store_true', help="Use virgin database backup")
