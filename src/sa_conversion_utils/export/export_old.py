...
EXCLUDE_COLUMNS = {"user_profile": ["email_signature"]}
...
def add_export_parser(subparsers):
    ...
    export_parser = subparsers.add_parser(
        "export", help="Export Postgres tables to CSV files."
    )
    export_parser.add_argument(
        "-s", "--server", default=env_config["PG_HOST"], help="Postgres server hostname"
    )
    export_parser.add_argument(
        "-d", "--database", default=env_config["PG_DB"], help="Postgres database name"
    )
    export_parser.add_argument(
        "-u", "--username", default=env_config["PG_USER"], help="Postgres username"
    )
    export_parser.add_argument(
        "-p", "--password", default=env_config["PG_PASSWORD"], help="Postgres password"
    )
    export_parser.add_argument(
        "-o", "--output", required=True, help="Path to output folder for CSVs"
    )
...
def export_postgres_to_csv(config: dict):
    ...
    logger.info(
        f"Exporting tables from '{database}' on host '{host}' into '{output_dir}'"
    )
...
def export_table_to_csv(host, database, username, table, output_dir):
    try:
        ...
        safe_columns = [
            f'"{col}"' if col in RESERVED_KEYWORDS else col for col in columns
        ]
